...
export default function Preloader({ onComplete }: PreloaderProps) {
  // Use a `mounted` flag so the first client render matches the SSR render
  // (both render `null`). After mount we check sessionStorage to decide
  // whether to show the preloader. Without this, the SSR render is the
  // full preloader markup while the client render (after the user has seen
  // it once) returns `null`, causing a hydration mismatch on the next
  // sibling (the Navbar `<header>`).
  const [mounted, setMounted] = useState(false);
  const [skipped, setSkipped] = useState(false);

  useEffect(() => {
    setMounted(true);
    setSkipped(!!sessionStorage.getItem(PRELOADER_SESSION_KEY));
  }, []);
  ...
  // Don't render anything before mount (matches SSR) or if already seen
