    ...
    <div className="flex gap-2 justify-center">
      {Array.from({ length }).map((_, i) => (
        <input
          ...
          disabled={disabled}
          className="w-11 h-13 text-center text-lg font-bold bg-background border border-border rounded-lg shadow-sm focus:border-primary/50 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all text-foreground"
...
function PasswordRequirements({ password, confirmPassword }: { password: string; confirmPassword?: string }) {
  ...
  return (
    <div className="space-y-1.5 p-3 rounded-lg bg-muted/50 dark:bg-secondary/30 border border-border">
...
export default function AuthDialog({ isOpen, onClose, defaultMode = 'login', onOpenLegal }: AuthDialogProps) {
  ...
  const hcaptchaTheme = mounted && resolvedTheme === 'dark' ? 'dark' : 'light';
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        const result = await loginWithEmailPassword(email, password, rememberMe, captchaToken);
        if (result.success) {
        ...
        <>
          ...
          >
            ...
            >
              ...
              <button
                onClick={onClose}
                className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground hover:text-foreground hover:bg-muted dark:hover:bg-secondary transition-all duration-200 z-10"
                      ...
                      <form onSubmit={handleForgotPasswordSubmit} className="space-y-4">
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <Input id="forgot-email" type="email" placeholder="you@example.com" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} className="pl-10 bg-background border border-border shadow-sm focus:border-primary/50 rounded-lg h-11" disabled={loading} />
                          </div>
                        </div>
                      ...
                      <form onSubmit={handleResetPasswordSubmit} className="space-y-4">
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <Input id="new-password" type={showPassword ? 'text' : 'password'} placeholder="Min 8 chars, upper, lower, number" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="pl-10 pr-10 bg-background border border-border shadow-sm focus:border-primary/50 rounded-lg h-11" disabled={loading} />
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                            <Input id="confirm-new-password" type={showPassword ? 'text' : 'password'} placeholder="Confirm your new password" value={confirmNewPassword} onChange={(e) => setConfirmNewPassword(e.target.value)} className={`pl-10 bg-background border border-border shadow-sm focus:border-primary/50 rounded-lg h-11 ${confirmNewPassword && confirmNewPassword === newPassword ? 'border-emerald-500/50' : ''}`} disabled={loading} />
                          </div>
                  ...
                  {step === 'form' && (
                    <motion.div key="form" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.2 }}>
                      ...
                      <div className="space-y-3 mb-5">
                        {/* Google OAuth */}
                        <Button type="button" variant="outline" disabled={loading || oAuthLoading !== null} onClick={() => handleOAuth('google')} className="w-full h-11 rounded-lg bg-background hover:bg-accent/50 border-border text-foreground font-medium transition-all duration-200 flex items-center justify-center gap-2.5">
                        ...
                        {/* GitHub OAuth */}
                        <Button type="button" variant="outline" disabled={loading || oAuthLoading !== null} onClick={() => handleOAuth('github')} className="w-full h-11 rounded-lg bg-background hover:bg-accent/50 border-border text-foreground font-medium transition-all duration-200 flex items-center justify-center gap-2.5">
                      ...
                      <form onSubmit={handleFormSubmit} className="space-y-4" noValidate>
                        ...
                        {mode === 'signup' && (
                          <div>
                            ...
                            <div className="relative">
                              ...
                              <Input id="auth-name" type="text" placeholder="Enter Your Full Name" value={name} onChange={(e) => setName(e.target.value)} onBlur={() => markTouched('name')} className={`pl-10 bg-background border border-border shadow-sm focus:border-primary/50 rounded-lg h-11 ${touched.name && errors.name ? 'border-destructive focus:border-destructive' : ''}`} disabled={loading} />
                            </div>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                            <Input id="auth-email" type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} onBlur={() => markTouched('email')} className={`pl-10 bg-background border border-border shadow-sm focus:border-primary/50 rounded-lg h-11 ${touched.email && errors.email ? 'border-destructive focus:border-destructive' : ''}`} disabled={loading} />
                          </div>
                        ...
                        {/* Remember Me & hCaptcha — login only */}
                        {mode === 'login' && (
                        ...
                        {mode === 'signup' && (
                          <div className="space-y-4">
                            ...
                            <div>
                              ...
                              <div className="relative">
                                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                                <Input id="auth-confirm-password" type={showPassword ? 'text' : 'password'} placeholder="Confirm your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} onBlur={() => markTouched('confirmPassword')} className={`pl-10 bg-background border border-border shadow-sm focus:border-primary/50 rounded-lg h-11 ${touched.confirmPassword && errors.confirmPassword ? 'border-destructive focus:border-destructive' : touched.confirmPassword && !errors.confirmPassword && confirmPassword ? 'border-emerald-500/50' : ''}`} disabled={loading} />
                              </div>
                              ...
                              <div className="relative" data-dropdown>
                                <div className="relative">
                                  ...
                                  <Input
                                    ...
                                    className={`pl-10 pr-9 bg-background border border-border shadow-sm rounded-lg h-11 ${touched.country && errors.country ? 'border-destructive' : country ? 'border-emerald-500/50' : ''}`}
                                    disabled={loading}
                                  />
                                  <button
                                    type="button"
                                ...
                                <div className="relative" data-dropdown>
                                  <div className="relative">
                                    ...
                                    <Input
                                      ...
                                      className={`pl-10 pr-9 bg-background border border-border shadow-sm rounded-lg h-11 ${touched.city && errors.city ? 'border-destructive' : city ? 'border-emerald-500/50' : ''}`}
                                      disabled={loading}
                                    />
                                    <button
                                      type="button"
                        ...
                        {/* Submit Button */}
                        <Button type="submit" disabled={loading || (!!HCAPTCHA_SITE_KEY && !captchaToken)} className="w-full bg-gradient-to-r from-primary to-accent text-primary-foreground font-semibold h-11 rounded-lg hover:shadow-lg transition-all duration-200">
