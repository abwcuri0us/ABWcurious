...
interface AuthContextType {
  ...
  loginWithEmailPassword: (email: string, password: string, rememberMe?: boolean, captchaToken?: string) => Promise<{ success: boolean; error?: string; needsVerification?: boolean; email?: string }>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loginWithEmailPassword = useCallback(async (email: string, password: string, _rememberMe = false, captchaToken?: string) => {
    try {
      const res = await fetch("/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, password, rememberMe: _rememberMe, captchaToken }),
        credentials: 'include',
      });
      const data = await res.json();
