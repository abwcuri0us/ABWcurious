...
// Sends a notification to the admin when a new contact form submission arrives.
// ---------------------------------------------------------------------------
...
/**
 * Sends an admin notification email when a new contact form is submitted.
 */
export async function sendContactNotification(payload: ContactPayload) {
  const adminEmail = process.env.ADMIN_NOTIFICATION_EMAIL ?? "info@abwcurious.com";
  const subject = `New Contact: ${payload.subject ?? "General Inquiry"} — ${payload.name}`;

  const htmlContent = `
    <h2 style="color:#1e293b;margin:0 0 20px">New Contact Form Submission</h2>
    <table style="width:100%;border-collapse:collapse;font-size:14px">
      <tr><td style="padding:8px 0;color:#64748b;width:120px;vertical-align:top"><strong>Name</strong></td><td style="padding:8px 0;color:#1e293b">${escapeHtml(payload.name)}</td></tr>
      <tr><td style="padding:8px 0;color:#64748b;vertical-align:top"><strong>Email</strong></td><td style="padding:8px 0"><a href="mailto:${escapeHtml(payload.email)}" style="color:#0891b2">${escapeHtml(payload.email)}</a></td></tr>
      ${payload.phone ? `<tr><td style="padding:8px 0;color:#64748b;vertical-align:top"><strong>Phone</strong></td><td style="padding:8px 0;color:#1e293b">${escapeHtml(payload.phone)}</td></tr>` : ""}
      ${payload.company ? `<tr><td style="padding:8px 0;color:#64748b;vertical-align:top"><strong>Company</strong></td><td style="padding:8px 0;color:#1e293b">${escapeHtml(payload.company)}</td></tr>` : ""}
      <tr><td style="padding:8px 0;color:#64748b;vertical-align:top"><strong>Subject</strong></td><td style="padding:8px 0;color:#1e293b">${escapeHtml(payload.subject ?? "General Inquiry")}</td></tr>
      <tr><td style="padding:8px 0;color:#64748b;vertical-align:top"><strong>Type</strong></td><td style="padding:8px 0"><span style="background:#e0f2fe;color:#0369a1;padding:2px 8px;border-radius:4px;font-size:12px">${escapeHtml(payload.contact_type ?? "general")}</span></td></tr>
    </table>
    <div style="margin:20px 0;padding:16px;background:#f8fafc;border-left:4px solid #0891b2;border-radius:4px">
      <strong style="color:#475569;display:block;margin-bottom:8px">Message:</strong>
      <p style="color:#1e293b;margin:0;line-height:1.6;white-space:pre-wrap">${escapeHtml(payload.message)}</p>
    </div>
    <p style="margin-top:24px"><a href="https://abwcurious.com/admin" style="background:#0891b2;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600;font-size:14px">View in Admin Panel</a></p>
  `;

  return sendNotificationEmail(adminEmail, subject, htmlContent);
}
