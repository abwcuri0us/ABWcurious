...
export const usersDb = {
  ...
  async update(id: string, updates: Record<string, unknown>) {
    ...
    if ('avatar' in mappedUpdates) {
      mappedUpdates.avatar_url = mappedUpdates.avatar;
      delete mappedUpdates.avatar;
    }
    
    const { data, error } = await supabaseAdmin.from('profiles').update(mappedUpdates).eq('id', id).select().single()
