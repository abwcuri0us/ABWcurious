...
import {
  ...
  ExternalLink, Filter, Package, GalleryHorizontal as ImageIcon, Brain as BrainIcon,
  BookOpen, Database, Upload, RefreshCw,
} from "lucide-react";
...
import StatusMaintenancePanel from "@/components/admin/StatusMaintenancePanel";
...
const SIDEBAR_SECTIONS = [
  ...
  { group: "CONTENT", items: [
    { id: "blogs-admin", label: "Blogs", icon: FileText },
  ...
  { group: "PRODUCTS", items: [
    { id: "products-admin", label: "Products", icon: Package },
  ...
  { group: "SYSTEM", items: [
    ...
    { id: "ai-management", label: "AI Management", icon: BrainIcon },
...
function GlassCard({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`rounded-xl border border-border bg-card backdrop-blur-sm ${className}`}>{children}</div>;
}
...
function AdminSidebarContent({
  ...
  return (
    <div className="flex flex-col h-full">
      <div className={`p-4 flex items-center gap-3 border-b border-border ${sidebarCollapsed ? "justify-center" : ""}`}>
...
export default function AdminDashboard() {
  /* eslint-disable react-hooks/rules-of-hooks -- hooks are all called unconditionally at component top-level */
      ...
      try {
        ...
        if (data.success && data.data?.user) {
          ...
          if (u.role === 'admin' || u.role === 'editor') {
            setUserInfo({ name: u.name || 'Admin', email: u.email, avatar: u.avatar || undefined });
            setIsAuthed(true);
          } else {
            setIsAuthed(false);
          }
  ...
  const handleLogout = async () => {
    try { await fetch("/api/auth/logout", { method: 'POST', credentials: 'include' }); } catch { /* continue */ }
  ...
  const TRow = ({ children, onClick }: { children: React.ReactNode; onClick?: () => void }) => (
    <tr className="border-b border-border hover:bg-muted/30 transition-colors cursor-pointer" onClick={onClick}>{children}</tr>
  );
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {quickActions.map((qa) => {
              const Icon = qa.icon;
              return (
                <Button key={qa.label} variant="outline" onClick={qa.action} className="h-auto py-3 flex flex-col items-center gap-2 border-border hover:bg-primary/5 hover:border-primary/30">
  ...
  const renderAnalytics = () => {
    ...
    const pageViews = [1240, 980, 1650, 890, 2100, 760, 1430];
    const signups = [12, 8, 24, 6, 31, 5, 18];
    const maxViews = Math.max(...pageViews);
    const maxSignups = Math.max(...signups);

    const topPages = [
      { path: "/", views: 3420, bounce: "42%" },
      { path: "/services", views: 1890, bounce: "38%" },
      { path: "/products", views: 1640, bounce: "51%" },
      { path: "/blogs", views: 1230, bounce: "29%" },
      { path: "/cybersecurity", views: 980, bounce: "44%" },
      { path: "/contact", views: 760, bounce: "22%" },
    ];

    const trafficSources = [
      { source: "Organic Search", pct: 42, color: "bg-emerald-500" },
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <AdminPageHeader title="Analytics" desc="Traffic, conversions, and user behaviour" />

        {/* KPI cards */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { label: "Page Views (7d)", value: "9,050", icon: Eye, change: "+14%", up: true, color: "text-cyan-400", bg: "bg-cyan-500/10" },
            { label: "Unique Visitors", value: "3,218", icon: Users, change: "+8%", up: true, color: "text-purple-400", bg: "bg-purple-500/10" },
            { label: "New Sign-ups", value: "104", icon: TrendingUp, change: "+22%", up: true, color: "text-emerald-400", bg: "bg-emerald-500/10" },
            { label: "Avg Session", value: "3m 42s", icon: Clock, change: "-5%", up: false, color: "text-amber-400", bg: "bg-amber-500/10" },
          ].map((kpi) => {
            const Icon = kpi.icon;
            return (
              <GlassCard key={kpi.label} className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className={`size-9 rounded-lg ${kpi.bg} flex items-center justify-center`}><Icon className={`size-4 ${kpi.color}`} /></div>
                  <span className={`text-xs font-semibold ${kpi.up ? "text-emerald-400" : "text-red-400"}`}>{kpi.change}</span>
                </div>
                <p className="text-2xl font-bold" style={{ fontFamily: "var(--font-space-grotesk)" }}>{kpi.value}</p>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          {/* Page Views Chart */}
          <GlassCard className="p-4">
            <h3 className="text-sm font-semibold mb-4">Page Views (7 Days)</h3>
            <div className="flex items-end gap-2 h-40">
              {days.map((d, i) => (
                <div key={d} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-[10px] text-muted-foreground">{pageViews[i].toLocaleString()}</span>
                  <div className="w-full relative" style={{ height: "100px" }}>
                    <motion.div initial={{ height: 0 }} animate={{ height: `${(pageViews[i] / maxViews) * 100}%` }} transition={{ delay: i * 0.05, duration: 0.5 }} className="absolute bottom-0 w-full rounded-t-md bg-gradient-to-t from-cyan-500/70 to-cyan-500/20" />
                  </div>
          ...
          {/* Sign-ups Chart */}
          <GlassCard className="p-4">
            <h3 className="text-sm font-semibold mb-4">New Sign-ups (7 Days)</h3>
            <div className="flex items-end gap-2 h-40">
              {days.map((d, i) => (
                <div key={d} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-[10px] text-muted-foreground">{signups[i]}</span>
                  <div className="w-full relative" style={{ height: "100px" }}>
                    <motion.div initial={{ height: 0 }} animate={{ height: `${(signups[i] / maxSignups) * 100}%` }} transition={{ delay: i * 0.05, duration: 0.5 }} className="absolute bottom-0 w-full rounded-t-md bg-gradient-to-t from-purple-500/70 to-purple-500/20" />
                  </div>
                  <span className="text-[10px] text-muted-foreground">{d}</span>
                </div>
              ))}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {/* Top Pages */}
          <GlassCard>
            <div className="p-4 border-b border-border"><h3 className="text-sm font-semibold">Top Pages</h3></div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead><tr><THead>Path</THead><THead>Views</THead><THead>Bounce Rate</THead></tr></thead>
                <tbody>
                  {topPages.map((p) => (
                    <TRow key={p.path}>
                      <TCell><code className="text-xs text-cyan-400">{p.path}</code></TCell>
                      <TCell><span className="font-semibold">{p.views.toLocaleString()}</span></TCell>
                      <TCell><span className="text-muted-foreground">{p.bounce}</span></TCell>
                    </TRow>
                  ))}
                </tbody>
              </table>
          ...
          <GlassCard className="p-4">
            ...
            <div className="space-y-4">
              {trafficSources.map((s) => (
                <div key={s.source}>
                  <div className="flex justify-between text-sm mb-1.5"><span className="text-muted-foreground">{s.source}</span><span className="font-semibold">{s.pct}%</span></div>
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${s.pct}%` }} transition={{ duration: 0.7 }} className={`h-full rounded-full ${s.color}`} />
                  </div>
                </div>
              ))}
            </div>
  ...
  // ── NEW SECTION: Blogs Admin ──
  const [adminBlogs, setAdminBlogs] = useState<{ id: string; title: string; slug: string; status: string; category: string; view_count: number; published_at: string; author?: { full_name: string } }[]>([]);
  const [blogsLoading, setBlogsLoading] = useState(false);

  const loadAdminBlogs = useCallback(async () => {
    setBlogsLoading(true);
    try {
      const res = await fetch("/api/blogs?limit=100&status=all");
      const data = await res.json();
  ...
  useEffect(() => {
    if (activeTab === "blogs-admin") loadAdminBlogs();
  }, [activeTab, loadAdminBlogs]);

  const renderBlogsAdmin = () => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <AdminPageHeader title="Blogs" desc="Manage all blog posts"
        action={
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={loadAdminBlogs} disabled={blogsLoading}><RefreshCw className={`size-4 mr-1 ${blogsLoading ? "animate-spin" : ""}`} />Refresh</Button>
            <Button size="sm" className="btn-glow" onClick={() => window.open("/blogs/write", "_blank")}><Plus className="size-4 mr-2" />Write Post</Button>
          </div>
        }
      />
      <GlassCard>
        <div className="p-4 border-b border-border">
          <div className="flex gap-3 items-center">
            <div className="relative flex-1 max-w-xs"><Search className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground" /><Input placeholder="Search posts..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 h-9" /></div>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr><THead>Title</THead><THead>Category</THead><THead>Status</THead><THead>Views</THead><THead>Date</THead><THead className="text-right">Actions</THead></tr></thead>
            <tbody>
              {blogsLoading && <tr><td colSpan={6} className="text-center py-8"><Loader2 className="size-5 animate-spin mx-auto text-primary" /></td></tr>}
              {!blogsLoading && adminBlogs.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-muted-foreground">No blog posts yet</td></tr>}
              {adminBlogs.filter(b => !searchQuery || b.title.toLowerCase().includes(searchQuery.toLowerCase())).map((blog) => (
                <TRow key={blog.id}>
                  <TCell><div className="font-medium max-w-xs truncate">{blog.title}</div><div className="text-xs text-muted-foreground">/blogs/{blog.slug}</div></TCell>
                  <TCell><Badge variant="outline" className="text-xs">{blog.category}</Badge></TCell>
                  <TCell><Badge variant={blog.status === "published" ? "default" : blog.status === "private" ? "secondary" : "outline"} className="text-xs capitalize">{blog.status}</Badge></TCell>
                  <TCell><span className="text-sm">{(blog.view_count ?? 0).toLocaleString()}</span></TCell>
                  <TCell className="text-muted-foreground">{fmtDate(blog.published_at)}</TCell>
                  <TCell>
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="sm" onClick={() => window.open(`/blogs/${blog.slug}`, "_blank")}><Eye className="size-3.5" /></Button>
                      <Button variant="ghost" size="sm" onClick={() => window.open(`/blogs/write?edit=${blog.id}`, "_blank")}><Pencil className="size-3.5" /></Button>
                      <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" onClick={() => confirmDelete("blogs", blog.id, blog.title)}><Trash2 className="size-3.5" /></Button>
                    </div>
                  </TCell>
                </TRow>
              ))}
  ...
  // ── NEW SECTION: Products Admin ──
  const PRODUCTS_DATA = [
    { id: "restaurant360", name: "Restaurant360", status: "Live", desc: "All-in-one restaurant management SaaS", category: "SaaS", users: "1,240+", mrr: "$4,800", href: "/products/restaurant360" },
    { id: "intelliqr", name: "IntelliQR", status: "Live", desc: "Dynamic QR codes with analytics", category: "SaaS", users: "890+", mrr: "$2,100", href: "/products/intelliqr" },
    { id: "cyberintelligence360", name: "CyberIntelligence360", status: "Beta", desc: "AI-powered cybersecurity platform", category: "Security", users: "64", mrr: "$1,200", href: "/products/cyberintelligence360" },
    { id: "studyspark", name: "StudySpark", status: "Soon", desc: "AI adaptive learning platform", category: "EdTech", users: "0", mrr: "$0", href: "/products/studyspark" },
  ];

  const renderProductsAdmin = () => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <AdminPageHeader title="Products" desc="Manage your product portfolio" />

      <div className="grid md:grid-cols-2 gap-4 mb-6">
        {PRODUCTS_DATA.map((prod) => (
          <GlassCard key={prod.id} className="p-5">
            <div className="flex items-start justify-between mb-3">
              ...
              <Badge variant="outline" className={`text-xs shrink-0 ml-2 ${prod.status === "Live" ? "text-emerald-400 border-emerald-400/30 bg-emerald-500/10" : prod.status === "Beta" ? "text-blue-400 border-blue-400/30 bg-blue-500/10" : "text-amber-400 border-amber-400/30 bg-amber-500/10"}`}>{prod.status}</Badge>
            </div>
            <div className="grid grid-cols-3 gap-2 mb-4 text-center">
              <div className="rounded-lg bg-muted/30 p-2"><p className="text-xs text-muted-foreground">Category</p><p className="text-xs font-semibold mt-0.5">{prod.category}</p></div>
              <div className="rounded-lg bg-muted/30 p-2"><p className="text-xs text-muted-foreground">Users</p><p className="text-xs font-semibold mt-0.5">{prod.users}</p></div>
              <div className="rounded-lg bg-muted/30 p-2"><p className="text-xs text-muted-foreground">MRR</p><p className="text-xs font-semibold mt-0.5 text-emerald-400">{prod.mrr}</p></div>
            </div>
            <div className="flex gap-2">
              <Button size="sm" variant="outline" className="flex-1" onClick={() => window.open(prod.href, "_blank")}><ExternalLink className="size-3 mr-1" />View Page</Button>
              <Button size="sm" variant="outline" className="flex-1"><Pencil className="size-3 mr-1" />Edit</Button>
            </div>
          </GlassCard>
        ))}
      </div>
      ...
      <GlassCard className="p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Total Products", value: "4", icon: Package, color: "text-cyan-400", bg: "bg-cyan-500/10" },
            { label: "Live Products", value: "2", icon: CheckCircle2, color: "text-emerald-400", bg: "bg-emerald-500/10" },
            { label: "Total Users", value: "2,194+", icon: Users, color: "text-purple-400", bg: "bg-purple-500/10" },
            { label: "Total MRR", value: "$8,100", icon: TrendingUp, color: "text-amber-400", bg: "bg-amber-500/10" },
          ].map((stat) => {
            const Icon = stat.icon;
            return (
              <div key={stat.label} className="flex items-center gap-3">
                <div className={`size-9 rounded-lg ${stat.bg} flex items-center justify-center shrink-0`}><Icon className={`size-4 ${stat.color}`} /></div>
                <div><p className="text-lg font-bold" style={{ fontFamily: "var(--font-space-grotesk)" }}>{stat.value}</p><p className="text-xs text-muted-foreground">{stat.label}</p></div>
              </div>
            );
          })}
        </div>
  ...
  // ── NEW SECTION: Media Library ──
  const [mediaFiles, setMediaFiles] = useState<{ name: string; url: string; size: number; created_at: string; type: string }[]>([]);
  const [mediaLoading, setMediaLoading] = useState(false);
  const [mediaFilter, setMediaFilter] = useState<"all" | "images" | "blogs">("all");

  const loadMediaFiles = useCallback(async () => {
    setMediaLoading(true);
    // In a real implementation, list from Supabase Storage
    // For now show placeholder data
    setMediaFiles([
      { name: "hero-banner.webp", url: "/og-image.png", size: 245000, created_at: new Date().toISOString(), type: "image" },
      { name: "blog-thumbnail.jpg", url: "/logo.svg", size: 128000, created_at: new Date().toISOString(), type: "blogs" },
    ]);
    setMediaLoading(false);
  }, []);

  useEffect(() => {
    if (activeTab === "media-library") loadMediaFiles();
  }, [activeTab, loadMediaFiles]);

  const renderMediaLibrary = () => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <AdminPageHeader title="Media Library" desc="Manage uploaded files and assets"
        action={
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={loadMediaFiles}><RefreshCw className="size-4 mr-1" />Refresh</Button>
            <Button size="sm" className="btn-glow"><Upload className="size-4 mr-2" />Upload Files</Button>
          </div>
        }
      />
      ...
      <div className="flex gap-2 mb-4">
        {(["all", "images", "blogs"] as const).map((f) => (
          <button key={f} onClick={() => setMediaFilter(f)} className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-all ${mediaFilter === f ? "bg-primary text-white border-primary" : "border-border text-muted-foreground hover:text-foreground"}`}>
      ...
      <GlassCard className="p-4">
        {mediaLoading ? (
          <div className="text-center py-12"><Loader2 className="size-8 animate-spin mx-auto text-primary" /></div>
        ) : mediaFiles.length === 0 ? (
          <div className="text-center py-12">
            <ImageIcon className="size-12 text-muted-foreground/30 mx-auto mb-3" />
            <p className="text-muted-foreground">No media files found</p>
            <p className="text-xs text-muted-foreground/60 mt-1">Upload images via the blog editor or use the Upload button above</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {mediaFiles.filter(f => mediaFilter === "all" || f.type === mediaFilter).map((file) => (
              <div key={file.name} className="group relative rounded-xl overflow-hidden border border-border hover:border-primary/30 transition-all cursor-pointer bg-muted/20">
                <div className="aspect-square bg-muted/30 flex items-center justify-center">
                  {file.url.match(/\.(png|jpg|jpeg|webp|gif|avif|svg)$/i) ? (
                ...
                <div className="absolute inset-0 bg-background/70 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                  <Button size="sm" variant="secondary" className="h-7 px-2 text-xs"><Eye className="size-3 mr-1" />View</Button>
                  <Button size="sm" variant="secondary" className="h-7 px-2 text-xs text-destructive"><Trash2 className="size-3 mr-1" />Del</Button>
                </div>
              </div>
            ))}
          </div>
  ...
  // ── NEW SECTION: AI Management ──
  const [kbEntries, setKbEntries] = useState<{ id: string; title: string; category: string; is_active: boolean; created_at: string }[]>([]);
  const [kbLoading, setKbLoading] = useState(false);
  const [kbForm, setKbForm] = useState({ title: "", content: "", category: "general", source_url: "" });
  ...
  const loadKB = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/ai/embed", { headers });
      const data = await res.json();
  ...
  useEffect(() => {
    if (activeTab === "ai-management") loadKB();
  }, [activeTab, loadKB]);

  const handleAddKBEntry = async () => {
    if (!kbForm.title || !kbForm.content) { toast.error("Title and content required"); return; }
    setKbSaving(true);
    try {
      const token = localStorage.getItem("abwcurious_token") ?? "";
      ...
      toast.success(`Entry embedded (${data.dimensions} dimensions)`);
      setKbDialogOpen(false);
      setKbForm({ title: "", content: "", category: "general", source_url: "" });
      loadKB();
    } catch (err) {
  ...
  const renderAIManagement = () => (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <AdminPageHeader title="AI Management" desc="Manage Curious AI knowledge base and monitor usage"
        action={<Button size="sm" className="btn-glow" onClick={() => setKbDialogOpen(true)}><Plus className="size-4 mr-2" />Add KB Entry</Button>}
      />

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: "KB Entries", value: String(kbEntries.length), icon: Database, color: "text-cyan-400", bg: "bg-cyan-500/10" },
          { label: "Active Entries", value: String(kbEntries.filter(e => e.is_active).length), icon: CheckCircle2, color: "text-emerald-400", bg: "bg-emerald-500/10" },
          { label: "AI Model", value: "Mistral", icon: BrainIcon, color: "text-purple-400", bg: "bg-purple-500/10" },
          { label: "Embed Dim.", value: "1024", icon: Cpu, color: "text-amber-400", bg: "bg-amber-500/10" },
        ].map((s) => {
          const Icon = s.icon;
          return (
            <GlassCard key={s.label} className="p-4">
              <div className="flex items-center gap-3">
                <div className={`size-9 rounded-lg ${s.bg} flex items-center justify-center shrink-0`}><Icon className={`size-4 ${s.color}`} /></div>
                <div><p className="text-xl font-bold" style={{ fontFamily: "var(--font-space-grotesk)" }}>{s.value}</p><p className="text-xs text-muted-foreground">{s.label}</p></div>
              </div>
            </GlassCard>
          );
        })}
      ...
      <GlassCard>
        <div className="p-4 border-b border-border flex items-center justify-between">
          <h3 className="text-sm font-semibold">Knowledge Base Entries</h3>
          <Button variant="ghost" size="sm" onClick={loadKB} disabled={kbLoading}><RefreshCw className={`size-3.5 ${kbLoading ? "animate-spin" : ""}`} /></Button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr><THead>Title</THead><THead>Category</THead><THead>Status</THead><THead>Created</THead><THead className="text-right">Actions</THead></tr></thead>
            <tbody>
              {kbLoading && <tr><td colSpan={5} className="text-center py-8"><Loader2 className="size-5 animate-spin mx-auto text-primary" /></td></tr>}
              {!kbLoading && kbEntries.length === 0 && (
                <tr><td colSpan={5} className="text-center py-10 text-muted-foreground">
                  <BrainIcon className="size-8 mx-auto mb-2 text-muted-foreground/30" />
                  <p className="text-sm">No knowledge base entries yet.</p>
                  <p className="text-xs mt-1">Add entries to enable RAG-powered responses in Curious AI.</p>
                </td></tr>
              )}
              {kbEntries.map((entry) => (
                <TRow key={entry.id}>
                  <TCell><div className="font-medium max-w-xs truncate">{entry.title}</div></TCell>
                  <TCell><Badge variant="outline" className="text-xs capitalize">{entry.category}</Badge></TCell>
                  <TCell>
                    <Badge variant={entry.is_active ? "default" : "secondary"} className={`text-xs ${entry.is_active ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : ""}`}>
                      {entry.is_active ? "Active" : "Inactive"}
                    </Badge>
                  </TCell>
                  <TCell className="text-muted-foreground">{fmtDate(entry.created_at)}</TCell>
                  <TCell>
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" onClick={() => confirmDelete("kb", entry.id, entry.title)}><Trash2 className="size-3.5" /></Button>
                    </div>
                  </TCell>
                </TRow>
              ))}
      ...
      <Dialog open={kbDialogOpen} onOpenChange={setKbDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader><DialogTitle>Add Knowledge Base Entry</DialogTitle><DialogDescription>This content will be embedded and used for Curious AI RAG responses.</DialogDescription></DialogHeader>
          <div className="space-y-4">
            <div><Label>Title</Label><Input value={kbForm.title} onChange={(e) => setKbForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. Services Overview" /></div>
            <div>
              <Label>Category</Label>
              <Select value={kbForm.category} onValueChange={(v) => setKbForm(f => ({ ...f, category: v }))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {["general", "services", "products", "company", "faq", "cybersecurity", "pricing"].map(c => <SelectItem key={c} value={c} className="capitalize">{c}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div><Label>Source URL (optional)</Label><Input value={kbForm.source_url} onChange={(e) => setKbForm(f => ({ ...f, source_url: e.target.value }))} placeholder="https://..." /></div>
            <div><Label>Content</Label><Textarea value={kbForm.content} onChange={(e) => setKbForm(f => ({ ...f, content: e.target.value }))} placeholder="Enter the knowledge content to embed..." rows={6} /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setKbDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleAddKBEntry} disabled={kbSaving} className="btn-glow">
              {kbSaving ? <Loader2 className="size-4 mr-2 animate-spin" /> : <BrainIcon className="size-4 mr-2" />}
              {kbSaving ? "Embedding..." : "Embed & Save"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
  ...
  const renderSection = () => {
    switch (activeTab) {
      ...
      case "blogs-admin": return renderBlogsAdmin();
      case "products-admin": return renderProductsAdmin();
      case "media-library": return renderMediaLibrary();
      ...
      case "case-studies": return <CaseStudiesPanel />;
  ...
  return (
    <div className="min-h-screen flex bg-background">
      {/* Desktop Sidebar */}
      <aside className={`hidden lg:flex flex-col border-r border-border bg-card/80 backdrop-blur-sm transition-all duration-300 ${sidebarCollapsed ? "w-16" : "w-60"}`}>
        <AdminSidebarContent
