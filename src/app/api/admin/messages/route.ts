...
import { supabaseAdmin, messagesDb, usersDb, notificationsDb, getUserFromToken } from '@/lib/supabase'
...
async function verifyAdminAccess(request: NextRequest) {
  ...
  if (!accessToken) return { authorized: false as const, user: null }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (otherUserId) {
      ...
      const unreadMsgs = messages.filter((m: Record<string, unknown>) => m.receiver_id === user.id && !m.is_read)
      for (const m of unreadMsgs) {
        await supabaseAdmin.from('private_messages').update({ is_read: true }).eq('id', m.id)
      }
