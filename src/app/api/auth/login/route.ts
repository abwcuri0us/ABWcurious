...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use admin getUserByEmail or iterate — the filter parameter is unreliable
    let existingAuthUser = null
    try {
      // Try to find user by iterating (listUsers filter is not standard)
      const { data } = await supabaseAdmin.auth.admin.listUsers({ perPage: 1000 })
      existingAuthUser = data?.users?.find(
        (u) => u.email?.toLowerCase() === email.toLowerCase()
      ) ?? null
    } catch {
      // If listUsers fails, we proceed to signInWithPassword which will handle it
    }
